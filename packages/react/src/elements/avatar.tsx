...
const renderAvatar = (type: string, load: boolean, size: string, name?: string) => {
    ...
    if (load) {
        if (name) {
            return <div className={clsx('text-white', SHORT_NAME[size])}>{name}</div>;
        }
...
const AvatarWrapper = ({ hasBorder, isRounded, className, style, children }: PropsWithChildren<AvatarWrapper>) => {
    ...
    return (
        <div className={clsx(className, rounded)} style={style}>
            {children}
        </div>
    );
};
...
export const Status = ({ className }: any) => {
    return <div className={clsx('absolute w-3 h-3 bg-green-500 rounded-full border-2 border-white', className)} />;
};
